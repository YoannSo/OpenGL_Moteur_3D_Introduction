...
void Camera::setPosition( const Vec3f & p_position )
	...
	void Camera::setLookAt( const Vec3f & p_lookAt )
	{
		...
		_right		  = glm::normalize( glm::cross( _up, _invDirection ) );
		_up			  = glm::cross( _invDirection, _right );
	...
	// le point observer par la camera serra  _position - _invDirection
	...
	Mat4f Camera::_getVPMatrix() { return this->_projectionMatrix * this->_viewMatrix;
	}
