...
namespace M3D_ISICG
{
	...
	Texture TriangleMeshModel::_loadTexture( const aiString & p_path, const std::string & p_type )
	{
		...
		if ( image.load( fullPath ) )
		{
			...
			//calcul du nombre de mipmaplevel a l'aide de la formule du cours si plus petit que 1 alors 1
			int nbLevel = log2( glm::max( image._width, image._height ) );
			if ( nbLevel < 1 )
				nbLevel = 1;

			//changement de certain filtre
			glTextureStorage2D( texture._id, nbLevel, internalFormat, image._width, image._height );
